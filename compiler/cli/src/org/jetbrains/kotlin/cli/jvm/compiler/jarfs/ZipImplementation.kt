...
fun BufferedRandomAccessFile.contentsToByteArray(
    ...
    //randomAccessFile.readFully(compressed, 0,  zipEntryDescription.compressedSize)
...
//                val q = ZipFile(this)
//                val x = q.getEntry(zipEntryDescription.relativePath)
//                val b = q.getInputStream(x).readBytes()
//
//                val w = Deflater(Deflater.DEFAULT_COMPRESSION, true)
//                val t = ByteArray(zipEntryDescription.compressedSize)
//                w.setInput(b)
//                w.finish()
//                val sz = w.deflate(t)
//
...
//                    if (i + sz > readBytes.size) continue
//                    if ((0 until sz).all { readBytes[i + it] == t[it] }) {
//                        qw = i
//                        break
//                    }
//                }
...
private fun RandomAccessFile.readIntLittleEndian(): Int {
    ...
    val c = this.read()
    val d = this.read()
    return (d shl 24) + (c shl 16) + (b shl 8) + (a shl 0)
}
