...
private const val JVM_STATIC = "JvmStatic"
...
object FirJvmStaticChecker : FirBasicDeclarationChecker() {
    ...
    private fun checkStaticNotInProperObject(declaration: FirDeclaration, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        declaration.reportOnProperParts {
            if (it.hasAnnotation(JVM_STATIC)) {
    ...
    private fun FirAnnotatedDeclaration.hasAnnotation(name: String): Boolean {
        return findAnnotation(name) != null
    }

    private fun FirAnnotatedDeclaration.findAnnotation(name: String): FirAnnotationCall? {
        return annotations.firstOrNull {
            it.calleeReference.safeAs<FirResolvedNamedReference>()?.name?.toString() == name
        }
    }
}
