...
internal class VarargInjectionLowering constructor(val context: KonanBackendContext): DeclarationContainerLoweringPass {
        ...
        element?.transformChildrenVoid(object: IrElementTransformerVoid() {
            ...
            override fun visitVararg(expression: IrVararg): IrExpression {
                ...
                val isConstantVararg = expression.elements.all {
                    it is IrConst<*> || it is IrStaticallyInitializedValue
                }
                if (isConstantVararg) {
                    return irBuilder.irCall(
                        ...
                        extensionReceiver = arrayHandle.createStatic(irBuilder, type, expression.elements.map {
                            when (it) {
                                is IrConst<*> -> irBuilder.irStaticallyInitializedConstant(it)
                                is IrStaticallyInitializedValue -> it
                                else -> throw IllegalStateException("Try to initialize vararg constantly, when it's imossible")
                            }
                        })
                    }
                }
    ...
    abstract inner class ArrayHandle(val arraySymbol: IrClassSymbol) {
        ...
        abstract fun createStatic(
                ...
                elementType: IrType,
                values: List<IrStaticallyInitializedValue>): IrStaticallyInitializedValue
    }
    ...
    inner class ReferenceArrayHandle : ArrayHandle(symbols.array) {
        ...
        override fun createStatic(
                ...
                elementType: IrType,
                values: List<IrStaticallyInitializedValue>): IrStaticallyInitializedValue {
            return builder.irStaticallyInitializedArray(
    ...
    inner class PrimitiveArrayHandle(primitiveType: PrimitiveType)
        ...
        override fun createStatic(builder: IrBuilderWithScope, elementType: IrType, values: List<IrStaticallyInitializedValue>): IrStaticallyInitializedValue {
            return builder.irStaticallyInitializedArray(
            ...
            val casted = values.map {
                ...
                when (it) {
                    is IrStaticallyInitializedConstant -> {
                        ...
                        builder.irStaticallyInitializedConstant(castedConst)
                    }
                    is IrStaticallyInitializedObject -> {
                        it.fields.values.single()
                    }
                    else -> error("unsigned integer can't be represented as ${it::class.qualifiedName}")
                }
            }
            return builder.irStaticallyInitializedObject(
                    arraySymbol.defaultType,
                    mapOf(
