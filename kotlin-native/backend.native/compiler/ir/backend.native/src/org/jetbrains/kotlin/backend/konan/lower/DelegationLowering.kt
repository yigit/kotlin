...
internal class PropertyDelegationLowering(val context: Context) : FileLoweringPass {
                    ...
                    return when (receiversCount) {
                        ...
                        2 -> error("Callable reference to properties with two receivers is not allowed: ${expression.symbol.owner.name}")
                            ...
                            val field = kProperties.getOrPut(expression.symbol.owner) {
                                kPropertyField(
                                        irExprBody(createKProperty(expression, this) as IrStaticallyInitializedValue),
                                        kProperties.size
                                )
                            }
    ...
    private fun createKProperty(
        ...
        return irBuilder.irBlock(expression) {
            ...
            val initializer = if (dispatchReceiver == null && extensionReceiver == null) {
                return irStaticallyInitializedObject(type, @OptIn(ExperimentalStdlibApi::class) buildMap {
                    put("name", irStaticallyInitializedConstant(name))
                    put("getter", irStaticallyInitializedIntrinsic(getterCallableReference))
                    if (setterCallableReference != null) {
                        put("setter", irStaticallyInitializedIntrinsic(setterCallableReference))
                    }
                })
    ...
    private fun createLocalKProperty(propertyName: String,
                                     ...
                                     irBuilder: IrBuilderWithScope): IrStaticallyInitializedValue {
        val symbols = context.ir.symbols
        return irBuilder.run {
            irStaticallyInitializedObject(
                    symbols.kLocalDelegatedPropertyImpl.typeWith(propertyType),
                    mapOf(
                            "name" to irStaticallyInitializedConstant(irString(propertyName)),
